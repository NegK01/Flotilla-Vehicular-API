    ...
    public function up(): void
    {
        ...
        DB::statement("
            ...
            DECLARE
                ...
                current_vehicle_status    VARCHAR;
            BEGIN
                -- Caso 1: Cancelación manual (status approved -> cancelled)
                -- Caso 2: Soft Delete (deleted_at NULL -> timestamp) de una solicitud aprobada
                IF (OLD.status = 'approved' AND NEW.status = 'cancelled') OR 
                   (OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL AND NEW.status = 'approved') THEN
                ...
                -- Caso 3: Restore (deleted_at timestamp -> NULL) de una solicitud aprobada
                ELSIF OLD.deleted_at IS NOT NULL AND NEW.deleted_at IS NULL AND NEW.status = 'approved' THEN
                    ...
                    -- Si no está fuera de servicio y no está en mantenimiento, forzar a 'reserved'
